...
class ValueIterationShuffleBudget:
	"""
    This class implements a MODEL-BASED MODULE, ie learning algorithm (value-iteration-shuffle-budget).
    ...
    There is a budget to limit the cost
    """
	...
	def __init__(self, experiment, map_file, initial_variables, action_space, boundaries_exp, parameters, options_log):
		...
		"""
		# --- INFER BUDGETS ---
		# Random inference phase
		self.infer_budget = parameters['infer_budget']
		...
		self.epsilon = 0.01 # 0.001 #boundaries_exp["epsilon"]
		...
		for state in self.map["transitionActions"]:
			...
			self.dict_decision["values"][s]={"history_decisions": [[0]*self.window_size,[0]*self.window_size,[0]*self.window_size,[0]*self.window_size,[0]*self.window_size,[0]*self.window_size,[0]*self.window_size,[0]*self.window_size]}
		...
		if self.log == True:
			...
			self.qvalues_evolution_log.write('{\n"logs" :\n['+json.dumps(list(self.dict_qvalues)))
	...
	def decide(self, current_state, qvalues): # TODO check budgetise (for other MB variants)
		"""
	...
	def infer(self, current_state):
		...
		while True:
			if convergence_indicator < self.epsilon or cycle >= self.infer_budget: # So that even if infer_budget = 0, we break
				break
			...
			if convergence_indicator < self.epsilon or cycle >= self.infer_budget:
				break
			...
			if self.not_learn == False:
				if action not in self.list_actions[previous_state]:
					...
					if (previous_state in self.dict_transitions["transitionActions"]) and (action in self.dict_transitions["transitionActions"][previous_state]['transition']) and (current_state in self.dict_transitions["transitionActions"][previous_state]['transition'][action]):
						...
						set_transitions_window(self.dict_transitions, previous_state, action, current_state,
											   self.window_size)
					# If the transition doesn't exist, add it
					else:
						add_transition(self.dict_transitions, previous_state, action, current_state, 0,
									   self.window_size)
	...
	def run(self, action_count, cumulated_reward, reward_obtained, previous_state, decided_action, current_state, do_we_plan): 
		...
		# Update the transition model and the reward model according to the learning.
			self.learn(previous_state, decided_action, current_state, reward_obtained) # Just probs and reward
		...
		for action in range(0,len(prefered_action)):
			for dictStateValues in self.dict_decision["values"]:
				if dictStateValues == current_state:
					prefered_action[action] = sum(self.dict_decision["values"][dictStateValues]["history_decisions"]
												  [action])
